...
variable is fairly large (around 14 Mb). To keep the pacea package a
reasonable downloadable size, the HOTSsea results are stored in a separate GitHub repository
...
suffixes `_min`, `_mean`, `_max`, and `_sd` TODO (check that gets included) are
additionally used here.

So, for example, `hotssea_avg0to30m_temperature_min` is calculated by TODO ask
Greig.
...
pdata <- hotssea_avg0to30m_temperature_mean()
```
...
```{r}
pdata
head(pdata[, 1:5])   # geometry column is included when selecting columns from
                     # an `sf` object.
```
