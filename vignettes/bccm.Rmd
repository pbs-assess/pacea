...
The results are available are TODO get from README, Travis has updated it.
...
## Spatial resolution of the bccm results in pacea - original but edited, see reordered one below TODO

Model results have been processed for `pacea` to be saved on an inshore 2km x 2km grid and
an offshore 6km x 6km grid, within the Canadian Exclusive Economic Zone
(EEZ). We briefly explain this before showing the BCCM variables available.

The areas for the two grids are the polygon data objects `inshore_poly` and
`offshore_poly`, and the combined area is the object `bccm_eez_poly`. These
objects use the 'NAD83/BC Albers' projection. The polygons are also `sf` (simple
feature) objects, which means that they have spatial and non-spatial
attributes. If you do not know much about `sf` objects then do not worry; the
template code explained in this vignette should be enought to allow you to plot
what you are interested in.

Let's view the combined area `bccm_eez_poly` object and plot all three objects:
```{r Polygons}
bccm_eez_poly  # this tells you that the co-ordinate reference systm (CRS)is NAD
               # 83/ BC Albers
...
Obviously, these will be more informative in relation to the BC coastline and Exclusive Economic
Zone (EEZ), which are saved as `bc_coast` and `bc_eez` objects, which we can
view and plot:
```{r}
bc_coast
bc_eez
...
Note that the `bc_coast` and `bc_eez` objects have the 'WGS 84' CRS, unlike the
earlier three objects that use 'NAD 83'. Fortunately, `ggplot()` recognizes
...
The mixed grid (2km x 2km and 6km x 6km) of cells can also be viewed, though the cells
may not be clearly visible if the resolution of the plot is too low
...
for example, `?bccm_eez_poly`.

## Spatial resolution of the bccm results in pacea - reordered version TODO (keep this or first one)
...
The function will provide the mean (median, sd, n) across the entire area, as well as the inshore and offshore statistics, based on the 200m isobath (i.e. continental shelf).
