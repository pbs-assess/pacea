...
To see where these are in relation to the BC coastline and Exclusive Economic Zone (EEZ), we have to reproject the `bc_coast` and `bc_eez` objects as they use the 'WGS 84' projection

```{r}
# transform to NAD 83/BC Albers
...
plot(inshore_poly, col = "green", add = TRUE)
plot(offshore_poly, col = "blue", add = TRUE)
plot(tbc_eez, lty = 2, add = TRUE)
plot(tbc_coast, col = "grey", add = TRUE)
```
...
NOTE: 
`ggplot()` recognizes that the projection of the ROMS data and the bc_coast and bc_eez shapefile are different, and will transform the layers to match the first layer. It also provides the axes units in degrees, regardless of projection.
