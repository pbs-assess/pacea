...
dummy to test pushing

```{r plot}
# TODO Move into data-raw and re-update data
class(buoy_sst) <- c("pacea_buoy",
                    class(buoy_sst))    # TODO change to pacea_buoy_sst and
                                        # rename plotting function
...
class(one)
plot(one)   # not working, had this before

one_with_NAs <- tidyr::complete(one,
                                sst_every_day = seq.Date(
                                  from = min(date),
                                  to = max(date),
                                  by = "day"),
                                fill = list(sst_every_day = NA))   # HERE HERE -
                                        # need sst somewhere
...
HERE - keep working on plot.pacea_buoy()
Need to use tidyr::complete() maybe with lubridate to fill in
NA's for missing dates, and then get ggplot not to join those up.
