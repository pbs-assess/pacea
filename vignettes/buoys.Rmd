...
plot(buoy_sst)
```
(Since `buoy_sst` has class `pacea_buoy`, `plot(buoy_sst)` function calls our tailored
`plot.pacea_buoy()` function.
...
We will probably adapt Andrea Hilborn's code (saved below in the .Rmd) to produce a panel plot of all buoys at
