...
```{r plot}
# TODO Move into data-raw and re-update data
class(buoy_sst) <- c("pacea_buoy_sst",
                    class(buoy_sst))
##'
##'  <desc>
##'
...
##' @
##' @}
plot.pacea_buoy_sst <- function(obj,
                                station = "C46146",    # plot one station if specified
                                # - if "all" or >1 then call this
                                # function repeatedly to make a grid plot
                                years = NULL      # plot a given year, or range, else do all
                                ){
  if(length(station == 1)){
    plot_buoy_sst_single(obj,
                         station = station,
                         years = years,
                         ...)    # maybe year_highlight, default is current
  } else {
...
# Andrea's time series plot:
library(ggplot2) # TODO add to all these functions probably
...
h <- g + geom_line() +
  labs(x = "Month",    # TODO make month, currently Julian day
       colour = "Year") +
...
hh <- h +
  geom_line(data = filter(one,
                          lubridate::year(date) ==
  lubridate::year(lubridate::today())),    # TODO add option for specifying year
...
TODO add
NA's into buoys_sst to specify missing data (check - may be in there and ggplot just joins up lines in between - nope: filter(one, is.na(sst)))
