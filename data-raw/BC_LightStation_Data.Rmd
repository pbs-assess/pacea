...
# Index 93 is the correct row from the BC_Partition_Objects$Mapping_Matrix
...
ggplot2::ggplot(data=
                  cbind(Partition_Polys,
                        (Lighthouse_SST_Salinity_DF %>%
                           filter(Year==1999, Month==4))) %>% 
                dplyr::select(Mean_Salinity_BC_Lightstation) %>%
                dplyr::mutate(Mean_Salinity_WCVI=
                         ifelse(row_number() %in%
                          ...
                          mean(Mean_Salinity_BC_Lightstation,na.rm=T),
                          NA)), 
