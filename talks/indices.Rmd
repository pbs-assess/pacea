...
Various climate and oceanographic indices are included in pacea, currently (TODO
needs tweaking)

```{r indicestable, echo=FALSE}

kableExtra::kable(pacea_indices, "latex")# %>%
...
object, and properly documented, described, and referenced in its help file, for example:
...
And say you want to see if specific events coincide with El Niño (based on a
true story, see vignette for details):
```{r onievent}
plot(oni,
...
which ensures that the `plot(oni)`
command above
