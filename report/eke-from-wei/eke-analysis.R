# See other .R, using Copernicus example. This one is too tailored for bccm output.
...
# TODO prob don't need parallel for EKE, but just leave in for now.
...
pacea_data_dir <- paste0(here::here(),
                         "report/eke-from-wei/")  # Where to save .rds
...
foreach(i = 1:length(nc_filenames)) %dopar% {
  ...
  nc_dat <- ncdf4::nc_open("eke_cmems_0.125_19930101-20260106.nc")
#                       paste0(pacea_dir,
#                         "/data-raw/roms/",
#                        this_filename))
